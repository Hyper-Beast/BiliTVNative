...
Android 的 `MediaCodecList` 只能说明设备声明了某种硬件解码能力，不能保证厂商解码器兼容该编码的全部码流。若部分视频可以立即播放，而另一些视频长时间缓冲或无法起播，可先在 `设置 -> 播放设置 -> 解码器` 中固定选择 `H.264`，再用同一视频复测。

以下设备已验证固定 H.264 可以规避自动模式选择 H.265 后的起播问题：

| 设备 | 系统 | ABI | 应用版本 | 现象与建议 |
| --- | --- | --- | --- | --- |
| Sony BRAVIA X9000H (`BRAVIA VH1`) | Android TV 12 / API 31 | `armeabi-v7a` | v1.0.0 | 自动模式选择 `OMX.MTK.VIDEO.DECODER.HEVC` 时，部分视频出现长时间缓冲，日志包含 `BadParameter`、`UnsupportedIndex` 和 fallback；固定 H.264 后同一视频正常起播 |
